...
void copy_loop(volatile uint16_t* src, uint16_t* dst, int len) {
    for (int i = 0; i < len && i < RF_CAPTURE_MAX_PULSES; i++) {
        dst[i] = src[i];
    }
}

void copy_memcpy(volatile uint16_t* src, uint16_t* dst, int len) {
    int copy_len = len;
    if (copy_len > RF_CAPTURE_MAX_PULSES) {
        copy_len = RF_CAPTURE_MAX_PULSES;
    }
    // We have to cast away volatile for memcpy, but since we are reading from
    // the capture buffer which has already been stabilized (captureLen = 0, capturing = false, noInterrupts()),
    // it's safe to cast away volatile.
