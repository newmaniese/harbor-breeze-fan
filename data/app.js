(function () {
  ...
  function appendRfLog(entry) {
    ...
    var msg = entry.recognized && entry.command
      ? 'Recognized: ' + escapeHtml(entry.command)
      : (entry.func8 ? 'Unrecognized (func ' + escapeHtml(entry.func8) + ')' : 'Unrecognized');
    ...
    fetch('/verify-tx?cmd=' + encodeURIComponent(cmd))
      ...
      .then(function (d) {
        ...
        var html = '<p><strong>TX seen by receiver:</strong> ' + seen + '</p>' +
          ...
          '<p>Seq before: ' + escapeHtml(d.seq_before ?? '') + ', Seq after: ' + escapeHtml(d.seq_after ?? '') + ', New capture during test: ' + newCap + '</p>';
